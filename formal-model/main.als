...
sig Perfil {}
abstract sig Status, Usuario {}
sig EmEspera, Apto, Rejeitado extends Status {}
sig Professor extends Usuario {}

sig Aluno extends Usuario { avaliacoes: set Avaliacao, perfil: one Perfil }
sig Projeto { responsavel: one Professor, alunos: set Aluno }
sig Candidatura { projeto: one Projeto, aluno: one Aluno, status: one Status }
sig Avaliacao { aluno: one Aluno, descricao: String, autor: one Professor }

fact UmPerfilPorAluno {
    all p: Perfil | one a: Aluno | a.perfil = p
}
fact TodoAlunoEmProjetoDeveTerCandidatura {
    all a: Aluno, p: Projeto | some c: Candidatura | (a in p.alunos) => (c.projeto = p and c.aluno = a)
}
fact AlunoRejeitadoEmProjetoNaoPodeSeInscreverNovamente {
    all c: Candidatura | all p: Projeto, a: Aluno | 
    a = c.aluno and p = c.projeto and c.aluno in c.projeto.alunos implies (c.status = EmEspera or c.status = Apto)
}
